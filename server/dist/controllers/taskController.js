...
export const updateTask = asyncHandler(async (req, res) => {
    ...
    const { title, description, project } = value;
    const task = await TaskModel.findOneAndUpdate({ _id: req.params.id, deletedAt: null }, { title, description, project }, { new: true, runValidators: true });
    if (!task) {
        return res.status(404).json({ message: 'Task not found' });
    }
    res.json(task);
...
export const updateTaskStatus = asyncHandler(async (req, res) => {
    ...
    const { status } = value;
    const task = await TaskModel.findOneAndUpdate({ _id: req.params.id, deletedAt: null }, { status }, { new: true, runValidators: true });
    if (!task) {
        return res.status(404).json({ message: 'Task not found' });
    }
    res.json(task);
