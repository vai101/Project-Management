...
export async function getProjectSummary(project: any, tasks: ITask[]): Promise<string> {
    const prompt = `
...
export async function getTaskAnswer(question: string, task: ITask, contextType: string): Promise<string> {
    const prompt = `
    Context: ${contextType}
    Task: ${task.title}
